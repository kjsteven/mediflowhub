...
if (isset($_SESSION['username'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['confirm_account_changes'])) {
            ...
            $stmtUpdateInfo = $conn->prepare("UPDATE doctors_table SET `First_Name` = ?, `Last_Name` = ?, Email = ?, `Phone_Number` = ?, Date_of_Birth = ? WHERE Email = ?");
            ...
            if ($_FILES['profile_image']['error'] === UPLOAD_ERR_OK) {  
                ...
                if (move_uploaded_file($tempFilePath, $newFilePath)) {
                    ...
                    $stmtUpdateImage = $conn->prepare("UPDATE doctors_table SET `Profile_Image` = ? WHERE Email = ?");
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['confirm_password_changes'])) {
            ...
            $stmt = $conn->prepare("SELECT Password FROM doctors_table WHERE Email = ?");
    ...
    $stmt = $conn->prepare("SELECT `Last_Name`, `First_Name`, Email, Password, `Phone_Number`, Date_of_Birth FROM doctors_table WHERE Email = ?");
    $stmt->bind_param("s", $username);
    $stmt->execute();
    $result = $stmt->get_result();
